...
class Flexi_auth extends Flexi_auth_lite
{
    ...
    public function webadmin_switch_login($identity = FALSE) 
	{
		if ($this->CI->flexi_auth_model->webadmin_switch_login($identity))
		{
			return true;
		}
	...
	public function resend_activation_token($identity) 
	{
		...
		else if ($this->CI->flexi_auth_model->deactivate_user($user_id))
		{
			...
			$first_name = '';//$user->{$this->CI->auth->database_config['custom']['user_profile']['custom_columns']['upro_first_name']};
			
			// Set email data.
			$email_to = $email;
			$email_title = 'Account Activation';
            
            $email_vars = $this->CI->config->item('VAR_EMAIL');
			...
			$user_data = array(
                ...
                'email_signature' => $email_vars['signature']
	...
	public function forgotten_password($identifier) 
	{
		...
		if ($this->CI->flexi_auth_model->forgotten_password($identity))
		{
			...
			$email_title = 'Set Takumi Mado Password';
	...
	public function insert_user($email, $username = FALSE, $password, $user_data, $group_id = FALSE, $activate = FALSE) 
	{
		...
		if ($user_id)
		{
			...
			$identity = $user->{$this->CI->auth->db_settings['primary_identity_col']};
			$activation_token = $user->{$this->CI->auth->database_config['user_acc']['columns']['activation_token']};
			$first_name = $user_data['upro_first_name'];
	 ...
	 * @author Rob Hussey
	 */
	public function send_email($email_to = FALSE, $email_title = FALSE, $template = FALSE, $email_data = array(), $attachements = FALSE)
	{
		...
		return $this->CI->flexi_auth_model->send_email($email_to, $email_title, $email_data, $template, $attachements);
	}
